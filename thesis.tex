\documentclass[faculty=fw,department=nat,phddegree=fys,10pt,final]{adsphd}
% \documentclass[faculty=fw,department=nat,phddegree=fys,10pt,online]{adsphd}


% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% !!                                                               !!
% !!  WARNING: do not remove the following lines between           !!
% !!  "%%% COVER: Settings %%%" and "%%% COVER: End settings %%%"  !!
% !!                                                               !!
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

%%% COVER: Settings %%%
\title{Computational modeling of biological nanopores}
%\subtitle{Multiscale computing for Dummies}

\author{Kherim}{Willems}

\supervisor{Prof.~dr.~ir.~P.~Van~Dorpe}{}
\supervisor{Prof.~dr.~G.~Maglia}{}
\supervisor{Prof.~dr.~J.~Hofkens}{}
\president{Prof.~dr.~E.~Carlon}{}
\jurymember{Prof.~dr.~ir.~T.~Stakenborg}{}
\jurymember{Prof.~dr.~ir.~M.~Roeffaers}{}
\jurymember{Prof.~dr.~C.~Bartic}{}
\externaljurymember{Prof.~dr.~rer.~nat.~U.~Keyser}{University of Cambridge, UK}

% Your research group within the department
% e.g. iMinds-DistriNet, Scientific Computing Group, ...
\researchgroup{Experimental Biophotonics}
\website{www.imec.be} % Leave empty to hide
\email{Kherim.Willems@imec.be} % Leave empty to hide

\address{Kapeldreef 75}
\addresscity{B-3001 Leuven} % This is the default value. Note
                              % that 'B-3001 Heverlee' is _incorrect_!!
                              % /[https://www.kuleuven.be/communicatie/marketing/intranet/huisstijl/taalgebruik.html]

\date{December 2020}
\copyyear{2020}
%\udc{XXX.XX}            % UDC, deposit number and ISBN are no longer necessary.
%\depot{XXXX/XXXX/XX}    % Leave out the initial D/ (it is added
                         % automatically)
%\isbn{XXX-XX-XXXX-XXX-X}


% Set spine width:
\setlength{\adsphdspinewidth}{15.25mm}

%% Set bleeds
%\setlength{\defaultlbleed}{7mm}
%\setlength{\defaultrbleed}{7mm}

% Set custom cover page
\setcustomcoverpage{mycoverpage.tex} % mycoverpage.tex is the default

%%% COVER: End settings %%%

% Verse package
\newcommand{\attrib}[1]{%
\nopagebreak{\raggedleft\normalsize #1\par}}


% PDFTeX page group warning
\pdfsuppresswarningpagegroup=1

% for the nomenclature (comment out if you do not use the nomencl package
\usepackage[cfg=thesis.cfg]{nomencl}   % For nomenclature
\renewcommand{\nomname}{List of Symbols}
\newcommand{\myprintnomenclature}{%
  \cleardoublepage%
  \printnomenclature%
  \chaptermark{\nomname}
  \addcontentsline{toc}{chapter}{\nomname} %% comment to exclude from TOC
}
\setlength{\nomitemsep}{-0.75\parsep}
\makenomenclature%

% for the list of abbreviations (comment out if you do not use the glossaries package
\usepackage[acronym, indexonlyfirst, nopostdot]{glossaries-extra} % For list of abbreviations
\newcommand{\glossname}{List of Abbreviations}
\newcommand{\myprintglossary}{%
  \renewcommand{\glossaryname}{\glossname}
  \cleardoublepage%
  \printglossary[title=\glossname]
  \chaptermark{\glossname}
  % \addcontentsline{toc}{chapter}{\glossname} %% comment to exclude from TOC
}
\renewcommand{\glossarypreamble}{\footnotesize}
\setabbreviationstyle{long-short}%
\makeglossaries%

% Additional document keys

% Define "is" key:
\glsaddkey*
{is}% key
{\glsentrytext{\glslabel}is}% default value
{\glsentryis}% command analogous to \glsentrytext
{\Glsentryis}% command analogous to \Glsentrytext
{\glsis}% command analogous to \glstext
{\Glsis}% command analogous to \Glstext
{\GLSis}% command analogous to \GLStext

% Define "ic" key:
\glsaddkey*
{ic}% key
{\glsentrytext{\glslabel}ic}% default value
{\glsentryic}% command analogous to \glsentrytext
{\Glsentryic}% command analogous to \Glsentrytext
{\glsic}% command analogous to \glstext
{\Glsic}% command analogous to \Glstext
{\GLSic}% command analogous to \GLStext


% BibLaTeX
\usepackage[utf8]{inputenc}
\usepackage{csquotes}
\usepackage[
  hyperref=auto,
  mincrossrefs=999,
  backend=biber,
  date=year,
  url=false,
  eprint=false,
  style=numeric,
  sorting=none
]{biblatex}
\addbibresource{allpapers.bib}

% Fonts

% \usepackage[T1]{fontenc}
% \usepackage{garamondx}
% \usepackage[garamondx,cmbraces]{newtxmath}

\usepackage{textcomp} % nice greek alphabet
\usepackage{pifont}   % Dingbats
\usepackage{booktabs, tabularx, multirow, threeparttable}
\usepackage{amssymb,amsthm}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{calligra}
\DeclareMathAlphabet{\mathcalligra}{T1}{calligra}{m}{n}
\usepackage{textgreek}



% science packages
\usepackage[version=4]{mhchem}
\usepackage{chemfig}

% units
\usepackage{siunitx}
	\sisetup{separate-uncertainty=true}
	\sisetup{multi-part-units=single}
	\sisetup{tight-spacing=true}
	\sisetup{inter-unit-product=\ensuremath{{}\cdot{}}}
	\sisetup{list-units=single}
  \sisetup{range-units=single}
  \sisetup{retain-explicit-plus=true}
  % \sisetup{retain-negative-zero=true}

\DeclareSIUnit \Molar {\textsc{M}}
\DeclareSIUnit \atm {\textsc{atm}}

% referencing
\usepackage{cleveref}
  \creflabelformat{equation}{#2#1#3}
  \crefname{figure}{Fig.}{Figs.}
  \Crefname{figure}{Figure}{Figures}
  \crefname{table}{Tab.}{Tabs.}
  \Crefname{table}{Table}{Tables}
  \crefname{equation}{Eq.}{Eqs.}
  \Crefname{equation}{Equation}{Equations}
  \crefname{section}{Sec.}{Secs.}
  \Crefname{section}{Section}{Sections}

% captions
\usepackage[font=scriptsize,labelfont=bf,labelsep=period]{caption}
  \captionsetup[table]{singlelinecheck=false}
  \captionsetup[table]{font=small}
  \captionsetup[table]{labelfont=bf}

\usepackage[font=footnotesize,labelfont=bf,labelsep=period,margin=1mm]{subcaption}
  \captionsetup[subfigure]{labelfont=bf}
  \captionsetup[subfigure]{textfont=normalfont}
  \captionsetup[subfigure]{labelformat=simple}
  \captionsetup[subfigure]{singlelinecheck=false}

\DeclareCaptionLabelFormat{bold}{\textbf{#2}}
  \captionsetup{subrefformat=bold}

% epigraphs (quotes)
\usepackage{epigraph}
\newcommand{\epipos}{80}
% \setlength{\epigraphwidth}{.382\textwidth}
\setlength{\epigraphwidth}{.4\textwidth}

% graphics
\usepackage{tikz}

% rotated pages
\usepackage{pdflscape}

% code blocks
\usepackage{listings}
\lstset{
  language=bash,
  basicstyle=\ttfamily
}

% hyphenation
\usepackage{hyphenat}

% afterpage
\usepackage{afterpage}

% colors
\usepackage[usenames,dvipsnames]{xcolor}
\usepackage{transparent}

\definecolor{greyblue}{RGB}{201,215,222}

\makeatletter 
\newlength{\figrulesep} 
\setlength{\figrulesep}{0.5\textfloatsep} 

\newcommand{\topfigrule}{\vspace*{-1pt}% 
\noindent{\color{greyblue}\rule[-\figrulesep]{\columnwidth}{1.5pt}} }

\newcommand{\botfigrule}{\vspace*{-2pt}% 
\noindent{\color{greyblue}\rule[\figrulesep]{\columnwidth}{1.5pt}} }

\newcommand{\dblfigrule}{\vspace*{-1pt}% 
\noindent{\color{greyblue}\rule[-\figrulesep]{\textwidth}{1.5pt}} }

\makeatother


% User commands

\input{mycommands.tex}

% Glossary and nomenclature files
\input{myglossary.tex}
\input{mynomenclature.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\makefrontcoverXII

\maketitle

\input{myquotepage}

\frontmatter % to get \pagenumbering{roman}

\includepreface{preface}
\includeabstract{abstract}
\includeabstractnl{abstractnl}

% To create a list of abbreviations, there are 2 options
% 1. manual creation of list of abbreviations and inclusion as a chapter
%    \includeabbreviations{abbreviations}
% 2. automatic generation via the glossary package
%    \glossary{name=MD,description=molecular dynamics}
\myprintglossary

% To create a list of symbols, there are 2 options
% 1. include a manually created nomenclature as a chapter
%    \includenomenclature{nomenclaturechapter}
% 2. automatic generation via the nomencl package
%    \nomenclature[cB]{$c_B(\vec{x})$}{Characteristic function of $B$}
\myprintnomenclature

\tableofcontents
\listoffigures
\listoftables

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\mainmatter % to get \pagenumbering{arabic}

% Show instructions on a separate page
\cleardoublepage

% Introduction
\includechapter{nanopores}
\includechapter{aims}

% Results
\includechapter{electrostatics}
\includechapter{trapping}
\includechapter{epnpns}
\includechapter{transport}

% Conclusions
\includechapter{conclusion}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \part{Appendices}
\appendix
\includeappendix{trapping_appendix}
\includeappendix{epnpns_appendix}
\includeappendix{transport_appendix}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\backmatter

\includebibliography
% BibTex
%\bibliographystyle{acm}
%\bibliography{allpapers}
% BibLatex (comment lines above and comment out biblatex lines in preamble)
\renewcommand*{\bibfont}{\small}
\printbibliography[title=\bibname]

\includecv{curriculum}

\includepublications{publications}

\makebackcoverXII

\end{document}
