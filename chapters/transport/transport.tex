...
\begin{itemize}
  \item K. Willems*, D. Rui\'{c}, F. L. R. Lucas, U. Barman, N. Verellen, G. Maglia and P. Van Dorpe.
        \textit{Nanoscale} \textbf{XX (X)}, XX--XX (2020) %\cite{Willems-2020}
\end{itemize}
%
\newpage
\end{shaded}
...
\begin{figure*}[t]
  ...
  {%
    ...
    modelling from the \textit{E. coli} \gls{clya} crystal structure (\pdbid{2WCD}~\cite{Mueller-2009}).
    Measurements indicate the average `cylindrical' sizes of the \cisi{} \lumen{} (\SI{6}{\nm} in diameter,
    \SI{10}{\nm} in height) and the \transi{} constriction (\SI{3.3}{\nm} in diameter, \SI{4}{\nm} in height).
    Images were rendered with \acrshort{vmd}~\cite{Humphrey-1996,Stone-1998}.
  %
...
\begin{figure*}[t]
  ...
  {%
    ...
    %
    The 2D-axisymmetric simulation geometry of \gls{clya-as} (grey) embedded in a lipid bilayer (green) and
...
\gls{rmsd} between their backbone atoms (\gls{vmd}'s \code{RMSD tool}~\cite{Humphrey-1996}).

\subsubsection{Side-chain B-factors.}
%

The thermal fluctuations of the side chains can be quantified with the temperature factor or \emph{B-factor}
...
\gls{md} structure, in comparison with the \gls{clya} dodecamer crystal \pdbid{2WCD}~\cite{Mueller-2009} and
...
from the CHARMM36 forcefield~\cite{Best-2012} and assigned using \code{PROPKA}~\cite{Olsson-2011} and
...
\begin{figure*}[p]
  ...
  %
  \caption[Measured and simulated ionic current through single {ClyA-AS} nanopores.]%
  {%
    %
    ...
    conductivity at the given concentrations. The grey envelopes represent the experimental errors (standard
    deviation, $n=3$).
    %
...
\begin{figure*}[p]
  ...
  \caption[Measured and simulated ionic conductance, rectification and cation selectivity of single {ClyA-AS}
  nanopores.]%
  {%
    %
    ...
    the simple resistor pore model. The grey envelopes represent the experimental errors (standard deviation,
    $n=3$).
    %
    (\subref{fig:transport_ioncurrent_icr_heatmap})
    %
    Contourplot of the ionic current rectification $\icr = \conductance(+\vbias)/\conductance (-\vbias)$,
    computed from the ionic conductances in the {ePNP-NS} simulation, as a function of $\vbias$ and $\cbulk$.
    %
    (\subref{fig:transport_ioncurrent_icr_concentration})
    %
    Comparision between the simulated ('ePNP-NS' and 'PNP-NS') and measured (expt.) $\icr$ as a function of
    ...
    \cref{tab:corrections_parameters}). The solid grey line represents $\tna=0.5$.
    %
...
The difference in ionic conduction at opposing bias voltages is also known as \gls{icr}
...
\subsubsection{Effect of individual corrections to the ionic and water conductance}
%
...
\begin{figure*}[t]
  ...
  {%
    ...
    \SI{-150}{\mV} (right), normalized over the values of the ePNP-NS equations. Comparison between the
    ...
    SMP: $\vec{\beta}=0$) and full {ePNP-NS}.
    %
...
\begin{figure*}[p]
  ...
  {%
    ...
    \lumen{} ($z = \mSI{5}{\nm}$) of \gls{clya}. The vertical line represents the the division between ions in
    the `bulk' ($\walldistance > \mSI{0.5}{\nm}$) of the pore and those located near its surface
    ($\walldistance \le \mSI{0.5}{\nm}$).
    %
...
FraC~\cite{Huang-2017,Huang-2019}, whereas the second is expected to be stronger for larger pores
...
\begin{figure*}[p]
  ...
  \caption[Concentration and voltage dependency of the electro-osmotic flow inside {ClyA-AS}.]%
  {%
    %
...
\subsubsection{{ClyA-AS} expression and purification.}
%
