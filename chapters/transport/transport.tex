...
from a 3D full atom homology model (\cref{sec:transport:model}). This is followed by the validation of the model
through a direct comparison of simulated and experimentally measured ionic conductances, and a thorough
...
\begin{figure*}[p]
  ...
  {%
    ...
    %
    The 2D-axisymmetric geometry was derived directly from the all-atom model using the radially averaged
...
the nanopore simulation geometry, after the manual removal of overlapping and superfluous vertices to improve
the quality of the final computational mesh. This 2D-axisymmetric geometry closely follows the outline of a
...
Describing a methodology for setting up a 3D model is beyond the scope of this chapter. We expect it to
...
its geometry (\ie~the diameter and length of the `resistor') but that it is strongly modulated by its
...
pore~\cite{Constantin-2007,White-2008,Wang-2014}. The latter can be either intrinsic to the geometry of nanopore,
...
inside the pore at low ionic strength may also explain why the \gls{epnp-ns} equations are
more accurate at higher ionic strengths ($\cbulk\ge\mSI{0.15}{\Molar}$). Because our ionic mobilities are
derived from bulk ionic conductances (\ie~for unconfined ions in a locally electroneutral environment) our
mobility model likely begins to break down under conditions where only a single ion type is
...
\gls{epnp-ns} equations, can accurately predict the ionic current flowing through \gls{clya} for a wide
...
total conductance. As expected from its negatively charged interior, we found \gls{clya} to be 
cation-selective (\ie~$\tna > 0.5$) for all investigated voltages up to a bulk salt concentration of 
$\cbulk \approx \mSI{2}{\Molar}$ \ce{NaCl} (0.5 contour line in \cref{fig:transport_ioncurrent_tsodium_heatmap}).
Above this concentration, $\tna$ falls to a minimum value of 0.45 at $\cbulk\approx\mSI{5}{\Molar}$,
...
measuring the reversal potential gives valuable insights into the selectivity of ion channels and small
...
Following the validation of the model in the previous section, we now proceed by describing the local ionic
...
\begin{figure*}[p]
  ...
  {%
    ...
    %
    Cross-section contour plots of the ion space charge density ($\scdion$), expressed as the number of elementary
...
Hence, on the one hand, they prohibit anions such as \Cl{} from entering the pore, and on the other hand, they
...
field~\cite{Soskine-2012}. The \gls{eof} is a consequence of the interaction between the fixed charges on the
...
second mechanism by coupling the Navier-Stokes and the Poisson-Nernst-Planck equations through a volume
...
the implementation of the wall distance corrections results in smaller effective pore sizes.  The influence
...
(\SI{\approx13.3}{\Molar} in our case), the steric corrections prevent the excessive screening of fixed charges by
...
their application to structurally more complex biological nanopores has been limited to date. We made use
...
pressure. These findings do not only provide valuable insights into the physical mechanisms of nanoscale
...
Experimental current-voltage curves were measured using single-channel electrophysiology, as detailed
...
Jose, USA). Data were acquired at \SI{10}{\kHz} and filtered using a \SI{2}{\kHz} low-pass filter. Measurements
