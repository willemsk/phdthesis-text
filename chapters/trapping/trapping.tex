...
%
%
\epigraphhead[80]{\epigraph{``Magic is organizing chaos. And while oceans of mystery remain, we have deduced that this requires two things. Balance and control.''}{
...
%
The text and figures of this chapter represent entirely my own work. All experimental work was performed by
Dr.~Annemie Biesemans, and the trapping model was derived by Dr.~rer.~nat.~Dino Rui\'{c}
(see~\cref{sec:trapping_appendix:escape_rates}). To the extent possible, the main and supplementary texts and
figures of the original manuscript were integrated into this chapter, and the remainder of the supplementary
information can be found in \cref{ch:trapping_appendix}.
%
%
\cleardoublepage
%
...
\caption[Trapping of proteins inside the ClyA-AS nanopore.]{%
  ...
  \glsreset{dhfr}
  %
  Depiction of a single \gls{dhfr} molecule extended with a positively charged C-terminal
  polypeptide tag (\DHFR{4}{S}) inside a \gls{clya-as} nanopore. The secondary structure of the tag (primarily
  ...
  fusion tag downward ($\forceep^{\rm{tag}}$), while the the electro-osmotic flow pushes the entire protein
...
\begin{figure*}[p]
  ...
  \begin{minipage}{6cm}
    ...
    %
  \end{minipage}
  \caption[Energy landscape of \DHFR{4}{S} inside ClyA.]{%
    \textbf{Energy landscape of \DHFR{4}{S} inside ClyA.}
    %
    (\textbf{\subref{fig:trapping_apbs_model}})
    %
    ...
    %
    %
    (\textbf{\subref{fig:trapping_apbs_energy}})
    %
    ...
    %
    %
    (\textbf{\subref{fig:trapping_apbs_barrier_body}})
    %
    ...
    %
    %
    (\textbf{\subref{fig:trapping_apbs_barrier_tag}})
    %
...
\begin{table}[t]
  \begin{threeparttable}
    ...
    %
    \caption[width=12cm]{Mutations and charges of all {DHFR} variants.}
...
\subsection{Engineering {DHFR}'s dwell time by manipulation of its charge}
%
...
need for MTX, several nonconserved amino acids on the surface of \DHFR{4}{S} were identified and mutated to
negatively charged glutamate residues, resulting in the molecules \DHFR{4}{I}, \DHFR{4}{C}, \DHFR{4}{O1}, and
\DHFR{4}{O2} (\cref{tab:dhfr_variants} and \cref{fig:trapping_dwell_times_body_model}). These mutations modify
the number of charges in the body compared to \DHFR{4}{S} and their charges are also in different locations.
...
\begin{table}[t]
  ...
  \begin{threeparttable}
    ...
    %
    \caption[width=12cm]{Fitting parameters for \DHFR{$\Ntag$}{O2}.}
...
properties of the system. Fitting this model to all \DHFR{$\Ntag$}{O2} data \emph{simultaneously}---with both
...
\begin{figure*}[p]
  ...
  \end{subfigure}
  %
  \caption[Electrostatic landscape of a {DHFR} bead model in ClyA-AS.]%
    {%
      \textbf{Electrostatic landscape of a {DHFR} bead model in ClyA-AS.}
      %
      ...
      of positive charges in the tag deepens the energetic mininum at $z \approx \mSI{3}{\nm}$.
      %
      %
...
\begin{figure*}[b]
  ...
  \caption[Tag charge dependence of the threshold voltage and translocation probability.]{%
    ...
    %
    Translocation probability voltage $\vbias^{P_{\rm{transl}}}$ plotted against tag charge for
    $P_{\rm{transl}} = \mSIlist{0.1;5;50;95;99.9}{\percent}$, shows that variants with high tag charge
    require less bias voltage to fully translocate the pore. Values were obtained through interpolation from
    \cref{eq:ptrans}, using the parameters in \cref{tab:fitting_params_complex}.
    %
...
\begin{table}
  ...
  \begin{threeparttable}[t]
    \centering
    %
    \caption[width=12cm]{Summary of all estimated threshold voltages and their corresponding dwell times.}
...
\gls{dhfr} was placed a various locations along the central axis of the pore using custom \code{Python} code
...
multiple Debye-H\"{u}ckel boundary conditions (\code{bcfl mdh}), followed by (2) a finer focussing calculation
...
\ce{Ni-NTA}-based affinity chromotography. After purification, \gls{clya-as} monomers were oligomerized in
...
preoligomerised \gls{clya-as} to the buffered electrolyte (\SI{150}{\mM} \ce{NaCl}, \SI{15}{\mM} \ce{Tris-HCl}
