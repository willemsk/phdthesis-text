...
protein-based, \emph{biological} nanopores (\glspl{bnp}) and the semiconductor-based, \emph{solid-state}
nanopores (\glspl{ssnp})~\cite{Dekker-2007}. This division runs deeper than the material however, as BNPs are
are formed in a bottom-up manner through self-assembly, while SSNPs are fabricated top-down using
micromachining techniques. Perhaps the most crucial difference between the two classes is the extent to which
they can be modified and manipulated. The proteiaceuous
...
\begin{figure*}[p]
  ...
  \begin{minipage}[t]{53mm}
    \begin{subfigure}[b]{53mm}
      \centering
      \caption{}\vspace{-8.5mm}\hspace{1.5mm}\label{fig:ahl_pore_side}
      \includegraphics[scale=1]{ahl_pore_side}
    \end{subfigure}
    %
    \vspace{5mm} \\
    %
    \begin{subfigure}[b]{53mm}
      \centering
      \caption{}\vspace{-8.5mm}\hspace{1.5mm}\label{fig:ahl_pore_top}
      \includegraphics[scale=1]{ahl_pore_top}
    \end{subfigure}
  \end{minipage}
  %
  \begin{minipage}{65mm}
    \begin{subfigure}[t]{53mm}
      \centering
      \caption{}\vspace{-8.5mm}\hspace{1.5mm}\label{fig:ahl_pore_section}
      \includegraphics[scale=1]{ahl_pore_section}
    \end{subfigure}
    %
    \vspace{5mm} \\
    %
    \begin{subfigure}[b]{32mm}
      \caption{}\vspace{-8.5mm}\hspace{1.5mm}\label{fig:ahl_monomer}
      \includegraphics[scale=1]{ahl_monomer}
    \end{subfigure}
    %
    \begin{subfigure}[b]{32mm}
      \centering
      \caption{}\vspace{-8.5mm}\hspace{1.5mm}\label{fig:ahl_protomer}
      \includegraphics[scale=1]{ahl_protomer}
    \end{subfigure}
  \end{minipage}
\caption[Structure of the \ta-hemolysin (\ta HL) porin.]{%
  \textbf{Structure of the \ta-hemolysin (\ta HL) porin.}
  %
  (\subref{fig:ahl_pore_side})
  %
  Side view and 
  %
  (\subref{fig:ahl_pore_top})
  %
  top view of the heptameric \gls{ahl} nanopore (\pdbid{7AHL}~\cite{Song-1996}), with a single subunit
  ...
  constriction.
  %
  (\subref{fig:ahl_monomer})
  %
  Crystal structure of the \SI{33}{\kDa} water-soluble \gls{ahl} monomer (\pdbid{4YHD}~\cite{Sugawara-2015})
  compared to that of
  %
  (\subref{fig:ahl_protomer})
  %
  the protomer (\pdbid{7AHL}~\cite{Song-1996}) in the final pore. While the the extracellular cap and the
...
The \ta-hemolysin (\gls{ahl}) protein (\cref{fig:ahl_pore_structure}) is a \tb-\gls{pft} secreted by
...
chemical and structural stability (\eg~temperature, pH, urea, mutations) and the availability of its crystal
...
%
The \gls{ahl} pore (\pdbid{7AHL}~\cite{Song-1996}) comprises seven identical subunits that form a
...
\cref{fig:clya_pore_structure})~\cite{Mueller-2009}, a large \ta-\gls{pft} secreted by various
...
contact with the solvent are formed by a core bundle of four tightly packed \ta-helices per protomer, and the
...
pore (\cref{fig:clya_pore_section}) and explains the observed cation
...
their increased aperture size, the overall structure and assembly mechanism of these pores is identical to that
...
effectively pushing them downwards and allowing them to `wedge' a hole into the lipid bilayer.
...
resolved in 2015 by Tanaka \etal{}~\cite{Tanaka-2015}, and surprisingly contained several co-crystallized
...
limited space available in the lumen of the final pore (\SI{\approx2}{\nm} diameter), it is likely that the
insertion of the N-terminal \ta-helix occurs in a non-concerted manner prior to full ring
closure~\cite{Cosentino-2016}. It is also possible that a pre-pore structure is formed, similar to the
non-lytic \gls{frac} nonamer~\cite{Mechaly-2011}, with the amphiphatic N-termini pushing through the membrane
in a concerted fashion~\cite{Tanaka-2015,Rojko-2016} much like the \gls{clya} porin. Evidence favors the first
...
\gls{cryo-em} image analysis and molecular modeling with a molecular weight in excess of
\SI{1000}{\kDa}~\cite{Lukoyanova-Kondos-2015}, making it one of the largest \glspl{pft} with a known structure
to date.
...
which is composed of two lipid binding \gls{plya} and a single pore forming \gls{plyb} protein chain
(\cref{fig:plyab_pore_side,fig:plyab_pore_top}).
...
water-soluble \gls{plyb} structure (\pdbid{4OEJ}~\cite{Lukoyanova-Kondos-2015}) can be subdivided in a
\tb-rich globular trefoil domain at the a C-terminus, and a \gls{macpf} domain at the N-terminus. At its core,
the latter contains a four-stranded bent and twisted \tb-sheet (typical for the \gls{macpf} superfamily), a
flexible \gls{hth}-motif and two \ta-helical regions (TMH1 and TMH2). In the protomer structure
(\pdbid{4VT2}~\cite{Lukoyanova-Kondos-2015}), the TMH1 and TMH2 regions become fully unwound into \tb-hairpins
to become part of the 52-sheet membrane-spanning \tb-barrel (\cref{fig:plyab_protomer}).
