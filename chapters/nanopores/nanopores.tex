...
\caption[Concept of single-molecule sensing with nanopores.]{%
  ...
  (blue) and anions (red) through the pore, resulting in a measureable `open pore' current ($\iopen$).
  %
  (Middle panel)
  %
  Entry of analyte 1 molecule (purple) into the water-filled channel of the pore from the \cisi{}-side
  transiently blocks the ionic current, until the molecule exits into the \transi{} reservoir (\ie~it
  `translocates'). This gives rise to the $\ilevel{1}$ current level, which has a characteristic `dwell time'
  $t_{\rm{d,1}}$.
  %
  (Right panel)
  %
  Similarly, entry of the larger analyte 2 molecule (green) into pore from the \cisi{} side leads to the
  deeper current blockade level $\ilevel{2}$ with a dwell time $t_{\rm{d,2}}$. Because of its size however, it
  cannot pass through the narrowest point of the channel and hence will eventually either remain trapped
  inside the pore, or eventually exit back into the \cisi{} reservoir.
  %
...
This (finally) brings us to back to the technique at hand, nanopore-based
...
Nanopores are not only extremely versatile, their inherently small size and relative simple read-out mechanism
(\ie~a current amplifier) makes them amenable to both minaturization and parallellization. Both of which are
exemplified by \gls{ont}'s \code{MinION} device, a handheld third generation sequencer powered by \num{512}
...
\textit{S. typhi}~\cite{Mueller-2009} (also known as HlyE) and \gls{frac} from \text{Actinia
...
\textphi29 phage, was modified to insert (reversibly) into lipid bilayers and shown to translocate
...
suitable materials include \ce{Si3N4}~\cite{Li-2001,Storm-2003}, \ce{SiO2}~\cite{Storm-2005},
...
proper protective layer, such as \ce{HfO2}, the size of a \ce{Si3N4} pore is known to `drift' due slow
...
\caption[Structure of the \ta-hemolysin (\ta HL) porin.]{%
  ...
  (protomer) highlighted in orange. The location of the lipid bilayer indicated in blue together with the
...
The \glsfirst{ahl} protein (\cref{fig:nanopores_ahl}) is a \tb-\gls{pft} secreted by \textit{Staphylococcus
aureus} in its post-exponential and stationary growth phases~\cite{Bhakdi-1991}. It shows a high affinity for
...
The \gls{ahl} pore (\pdbid{7AHL}~\cite{Song-1996}) comprises of seven identical subunits of 293~amino acids
...
In 2009, Mueller \etal{} revealed the crystal structure of \glsfirst{clya},
...
monomers (\ce{M}) undergo a rapid transition ($t_{1/2} \approx \SI{80}{\second}$) to molten globule-like
intermediate (\ce{I}), followed by a slow assembly  ($t_{1/2} \approx \SI{1000}{\second}$) of the
membrane-bound protomers (\ce{P})into the full dodecameric pore (\ce{P_12})~\cite{Eifler-2006}. However,
...
\caption[Structure of the Fragaceatoxin C (FraC) porin.]{%
  ...
  (\pdbid{4TSY}~\cite{Tanaka-2015}) with single protein subunit highlighted in color and the co-crystallized
  ...
  domain (orange). Displacement of Phe16 (mauve) from its hydrophobic pocket by Val60 (gray) trigger pore
  formation.
  %
...
\Glsfirst{frac}, \cref{fig:nanopores_frac}) is an \ta-\gls{pft} belonging to the family of the actinoporins,
and is produced by the sea anemone \textit{Actinia fragacea}. Its X-ray structure was resolved in 2015 by
Tanaka \etal{}~\cite{Tanaka-2015}, and surprisingly contained several co-crystallized sphingomyelin (\gls{sm})
lipids per subunit. This indicates that \gls{frac} does not merely exhibit a high degree of specificity
towards \gls{sm}-containing membranes, these lipids comprise an essential structural element of the pore
itself. Over the past 4 years, \gls{frac} has become a popular nanopore for the analysis of
peptides~\cite{Huang-2017,Huang-2019} and amino acid~\cite{Restrepo-Perez-2019a}, making it an interesting
candidate for protein sequencing applications~\cite{Restrepo-Perez-2018}.
...
(\pdbid{4TSY}~\cite{Tanaka-2015}), while the N-terminal domain forms a much longer \ta-helix that spans the
...
Even though the precise mechanisms with which actinoporins---including \gls{frac}---form pores is still under
...
\Glsfirst{plyab}, \cref{fig:nanopores_plyab}) is a \tb-\gls{pft}, secreted by the fungus \textit{Pleurotus
...
Regardless of the precise details of the sensing approach, before any analyte molecule can be detected it must
first be attracted towards the pore, and subsequently captured for a period of time sufficiently long to be
sampled. In the following sections, we will describe the origin of the forces at play in nanopores at all
stages of the sensing process, and discuss their respective relevance for different analytes. 
...
\caption[Schematic overview of the structure of the electrical double layer (EDL).]{%
  ...
  deplete. This region of excess charge in the fluid is called the \gls{edl}, and has a characteristic
  thickness equal to the Debye length ($\dbl$, see~\cref{eq:nanopores_debye_length}). Counterions in the Stern layer are
  semi-permanently bound to the surface and partially lose their hydration shell. Because the surface
  electrostatic potential drops exponentially inside the electrical double layer, its influence becomes
  neglible after a few Debye lengths, rapidly resulting in an electroneutral bulk ionic distribution.
  %
...
physiological salt concentration (\eg~\SI{0.15}{\Molar}), a volume of \grid{10}{10}{10}{\cubic\nm} contains
...
\caption[Equivalent circuit model for the electric potential and field in a nanopore system.]{%
  \textbf{Equivalent circuit model for the electric potential and field in a nanopore system.}
  %
  ...
  difference between the \cisi{} and \transi{} electrodes is not exactly, $\potdiff$, but is given mopre
  ...
  resistance, a portion of $\potdiff$ decays outside of the pore in close proximity to its entries, which
  helps with the capture of analytes. However, the bulk of the potential change occurs within the pore itself,
  which results in a drastic increase of the electric field strength. Note that most biological nanopores are
  not perfect cylinders and hence will have non-uniform electric fields along their length.
  %
...
\caption[Electro-osmotic flow in a nanopore system.]{%
  \textbf{Electro-osmotic flow in a nanopore system.}
  %
  ...
  molecules, giving rise to an \acrfull{eof} with a net water velocity in the z-direction ($\velocity_z(y)$).
  Note that while in hydropholic nanopores, $\velocity_z(y)\approx0$ at the wall, a non-zero velocity will be
  ...
  for a perfectly cylindrical pore (\cref{eq:nanopores_eof_velocity}). Similar to the electric field, the decay
  of $\velocity (z)$ is inversely proportional to the square of the distance from the pore.
  %
  }\label{fig:nanopores_eof}
\end{figure*}
%

As with the electrical field, tractable analytical formula's that accurately describe the \gls{eof} generated
...
The externally applied bias voltage creates a strong electric field that field that will result in several
distinct forces that influence the movement of analyte molecules far away from, and particularly within, the
pore (\cref{fig:nanopores_forces_ext}): the \glsdisp{ep}{electrophoretic~(EP)},
\glsdisp{eo}{electro-osmotic~(EO)} and \glsdisp{dep}{dielectrophoretic~(DEP)} forces. The former two are the
...
\caption[Forces acting on a particle due to the external electric field.]{%
  ...
  %
  Because the electric field is strongest within the pore, both the \emph{electrophoretic} and
  \emph{electro-osmotic} forces are at their strongest during particle translocation. Note that, depending on
  the charge of the particle and the direction of the \gls{eof} (\ie~the charge of the pore walls), these
  forces can either reinforce, or oppose one another.
  %
  (\subref{fig:nanopores_forces_ext_dep})
  %
  At locations where the electric field in not uniform (\ie~$\nabla \efield \neq 0$), such as near the pore
  entries or near a constriction within the pore, dielectric particles with a permanent or induced dipole
  moment will be subjected to a \emph{dielectrophoretic} force, which pushes them towards increasing electric
  field magnitudes.
  %
...
\caption[Forces acting on a particle due to the intrinsic interactions.]{%
  ...
  %
  The intrinsic \emph{electrostatic} interactions between charged atoms on the translocating particle and the
  ...
  %
  If the diameter of the particle closely matches that of the pore, the confinement leads to \emph{steric}
  clashes (\ie~repulsive intramolecular interactions) between the particle and the nanopore walls. In the case
  of proteins, this may prevent them from moving through the pore in their native state, resulting in unfolded
  translocation, or force them to assume a specific conformation or orientation---both of which induce an
  \emph{entropic} penalty.  
  %
...
Electrostatic forces arises when two charged atoms interact with one another, either in a repulsive (like-like
...
\glsfirst{mc} simulations, \glsfirst{md}, and stochastic dynamics~\cite{Paquet-2015}. Whereas the \gls{mc}
...
(\ie~no inertia) it is known as \glsfirst{bd}. Because we will only discuss \gls{md} in detail, the interested
...
% what is MD?
%  - solve equations of motion for a complex, atomic system
%     - bonded + non-bonded forces
%     - algoritms
%  - force fields
% advantages
%  - typically considered to be the most realistic approach for simulating molecular systems
%  - limited set of assumptions needed
%  - observe dynamic changes of molecular system
% disadvantates
%  - very computationally expensive
%  - prone to artifacts (water, protein unfolding)
%  - data analysis
...
in the system (typically hydrogen bond vibration), leading to the use of timesteps in the order of \SI{1}{\fs}
(\SI{e-15}{\second}). This places a practical limit of \SIrange{0.1}{1}{\us} on the length trajectories of
large molecular assemblies. For atomistic systems with \num{\approx e6}~atoms (\eg~a large biological
nanopore), the upper limit  for long-timescale trajectories (\ie~computational times within the duration of
the typical doctoral program), using current algorithms and supercomputers, is \SI{\approx
...
\glsdisp{pnp-ns}{PNP-NS} theory to improve their validity at the nanoscale and enable accurate, quantitative
...
\footnotetext{
  %
  The Einstein relation is $\mobility = \dfrac{\echarge \chargen}{\boltzmann \temperature} \diffusion$ .
  %
}
%
...
The field of single-molecular sensing has grown tremendously over the past two decades. Nanopores have
