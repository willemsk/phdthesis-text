...
Next, we switch to the much smaller \gls{wtfrac} (wild-type) and \gls{refrac} pores, which also have oppositely
...
mutations, but its missing atoms were also added with \code{psfgen}. Both structures were minimized with an
\gls{md} run (implicit solvent, CHARMM36 force field~\cite{Best-2012}) of \SI{100}{\ps} using \gls{namd}, with
...
the relevant mutations with \code{psfgen}: \gls{clya-r} (S110R), \gls{clya-rr} (S110R/D64R), \gls{clya-rr56}
...
\caption[Effective $\pKa$ values of {WtFraC} and {ReFraC}]{%
  ...
  identical between these two variants, only residues 10 and 159 were plotted for \gls{refrac}. The solid
...
\caption[Electrost. energy of ssDNA transl. through {WtFraC} and {ReFraC}]{%
  ...
  locations of the horizontal slices are indicated using a gray line (\sliceline).
  %
  Computations were performed using \gls{apbs}~\cite{Baker-2001,Baker-2005}.
  %
  Molecular representations were rendered with \gls{vmd}~\cite{Humphrey-1996,Stone-1998}.
...
\footnotetext{
  ...
  each the result of its own (random) energy minimization, not all of the non-mutated side-chains are necessarily
...
\caption[Electrost. energy of {dsDNA} transl. through {ClyA} at physiol. ionic strength]{%
  ...
  (\SI{65}{\angstrom}, bottom). The locations of the horizontal slices are indicated using a gray line
  (\sliceline).
  %
  Computations were performed using \gls{apbs}~\cite{Baker-2001,Baker-2005}.
  %
...
\caption[Electrost. energy of {dsDNA} transl. through {ClyA} at high ionic strength]{%
  ...
  (\SI{25}{\angstrom}, bottom). The locations of the horizontal slices are indicated using a gray line
  (\sliceline).
  %
  Computations were performed using \gls{apbs}~\cite{Baker-2001,Baker-2005}.
  %
