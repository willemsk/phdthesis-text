...
%
%
\chapter{The electrostatics of biological nanopores}\label{ch:electrostatics}
%
%
\epigraphhead[80]{\epigraph{``The sciences are monuments devoted to the public good; each citizen owes to them
a tribute proportional to his talents. While the great men, carried to the summit of the edifice, draw and put
up the higher floors, the ordinary artists scattered in the lower floors, or hidden in the obscurity of the
foundations, must only seek to improve what cleverer hands have created.''}{
...
demonstrate the extent with which their electrostatic potentials can be manipulated by modifcation of specific
amino acids of the pore on the one hand, and by altering the ionic strength and the pH of the electrolyte on
the other. Additionally we use electrostatic energy computations to analyze the translocation of \gls{ssdna}
and \gls{dsdna} through respectively \gls{frac} and \gls{clya} nanopores.
%
\\
%
...
As one of the four fundamental forces of nature, electromagnetism dominates the behaviour and properties of
...
strong \cite{Walker-2011}. Here, we are interested in the electrostatic component of the electromagnetic force
(\ie~the static interaction between fixed and mobile charges) and the electrostatic potential and energies
that result from it.
...
in the \gls{kcsa} channel \cite{Kopec-2019}. This behaviour is not limited to ion channels however, and can
...
\SIlist{0.15;2.5}{\Molar}. These mutants showed a varying ability to translocate \gls{dsdna}
\cite{Franceschini-2016}, a result corroborated by our electrostatic potential and \gls{dsdna} translocation
electrostatic energy calculations. Finally, we show that the strong repulsion between the negatively charged
constriction and the \gls{dna} confines the latter to the center of the pore \cite{Bayoumi-2020}.
...
\paragraph{Homology modelling of the full-atom pore structure.}
%
...
subunits, and for the bulk of the residues in \gls{plyb} protomers (\ie~everthing except the pore-forming
...
and two conjugate gradients energy minimizations of 200 steps with and without density contraints. Further
...
%
The docecameric \textit{E. coli} \gls{clya} crystal structure (\pdbid{2WCD} \cite{Mueller-2009}) contains all
...
calculations except the \gls{frac} only pore, where we used the \gls{parse} force field
\cite{Sitkoff-1994,Sitkoff-1996}. During conversion, \gls{pdb2pqr} assigns a protonation states to all
...
analytically with a Debye-H\"{u}ckel model for a multiple, non-interacting spheres containing point charges
\cite{Baker-2001,Baker-2005,Stone-2010}. The full list of simulation parameters can be found in
...
\begin{table}[p]
  ...
  \begin{tabularx}{18cm}{%
    ...
    %
    Name/Symbol & Unit & Dom.$^\text{\emph{a}}$
    ...
    %
    $\permittivity_{r,\text{electrolye}}$ & 1 
      & E
  ...
  \end{tabularx}
  \begin{flushleft}
    $^\text{\emph{a}}$Domains: global (G), electrolyte (E), protein/DNA (P), lipid bilayer (B);
    $^\text{\emph{b}}$pH values used by \gls{propka} during \gls{pdb2pqr} runtime;
  \end{flushleft}
\end{table}
\end{landscape}
%
...
enetgy,\cite{Bonthuis-2006} its inclusion in \gls{apbs} comes at a significant computational cost and its
...
effective $\pKa$'s up or down, depending on wether the de(protonated) state is energetically favorable or not,
which may even cause residues to be partially (de)protonated at neutral pH.
...
\gls{pdb2pqr} at \pH{1}), (2) the per-residue $\pKa$ values (calculated by either \gls{propka}
\cite{Olsson-2011} or \gls{delphipka} \cite{Wang-2015}), (3) a user-defined pH value and (4) the force-field
of choice. This approach was used to compute the electrostatics of the \gls{frac} variants at \pH{4.5} and
...
When using the \gls{pbe} to evaluate the electrostatic of a system, its electrostatic energy is given by
\cite{Baker-2005}
%
...
\caption[Computing net electrostatic energies with APBS.]{%
  ...
  $\gibbs^{\rm{pore+part}}$ (left) by substracting the energies of the empty pore $\gibbs^{\rm{pore}}$
...
\paragraph{Size matters: large proteins require large nanopores.}
%
...
proximity to the entry of pore \cite{Movileanu-2000,Fahie-2015,Ho-2015,Laszlo-2017,Thakur-2019}. Larger
nanopores such as \gls{clya}, allow for the immobilization of the entire protein \emph{within} the \lumen{} of
the nanopore itself
\cite{Soskine-2012,Soskine-2013,Soskine-Biesemans-2015,Biesemans-2015,Wloka-2016,VanMeervelt-2017,Galenkamp-2018,Galenkamp-2020},
...
nanopores is that, for more than two decades, nanopore research was focussed primarily towards \gls{dna}
...
C94S; \gls{plyb} N26D, K255E, E260R, E270R, A328T, C441A, A464V; \cref{fig:plyab_mutants_residues}, right)
\cite{Huang-2020}. Notably, \gls{plyab-r} showed significantly altered behaviour w.r.t. capture of negatively
charged proteins. In \gls{plyab-e2}, blockades are only observed for the relatively small (\SI{24}{\kDa})
\tb-casein when it was added at the \transi{} side (at positive bias voltages), and not for the larger
(\SI{66.5}{\kDa}) \gls{bsa}. \gls{plyab-r}, on the other hand, exhibited blockades for both \tb-casein and
\gls{bsa}, and from either the \cisi{} (at positive bias voltages) or \transi{} (at positive bias voltages)
side. In the following section we will describe the electrostatics of these \gls{plyab} variants, discuss the
implications of their differences, and finally link them to the experimentally observed differences.
...
\paragraph{Charge reversals induce potential reversals.}
%
...
\paragraph{An electro-osmotic tug of war.}
%
...
the electro-osmotic force, and give rise to the capture behaviour observed experimentally \cite{Huang-2020}.
...
%
As with other nanopores, the narrowest location in \gls{frac}'s hydrophillic channel is expected to dominate
...
mutation at the bottom of its constriction, showed a much lower conductance (cfr.~$\conductance =
\mSI{1.91(17)}{\nS}$ (\gls{wtfrac}) and $\conductance = \mSI{1.19(12)}{\nS}$ (\gls{refrac}), at
\SI{+50}{mV} and \SI{1}{\Molar} \ce{NaCl} \cite{Wloka-2016}) and a reversed ion selectivity
(cfr.~$\permeabilityratio{\ce{K+}}{\ce{Cl-}} = 3.6$ (\gls{wtfrac}), $\permeabilityratio{\ce{K+}}{\ce{Cl-}} =
...
\gls{refrac}, exhibted different peptide capture dynamics at low pH values \cite{Huang-2017}.
...
\caption[Effective $\pKa$ values of Wt- and ReFraC.]{%
  ...
  (D10R/K159E, right), colored according to their subunit-averaged $\pKa$ values, as computed by \gls{propka}
  \cite{Olsson-2011,Sondergaard-2011}.
  %
  (\subref{fig:frac_mutants_pka_scatter})
  %
  Scatterplot of the chain-averaged $\pKa$ values of all titratable residues of \gls{wtfrac} (red outlines)
  ...
  lines indicate the baseline $\pKa$ values of the given amino acids used by \gls{propka}
  \cite{Olsson-2011,Sondergaard-2011}.
  %
  Molecular structures were rendered using VMD \cite{Humphrey-1996,Stone-1998}.
  %
  }\label{fig:frac_mutants_pka}
\end{figure*}
%

\subsubsection{The pH-dependence of Wt- and ReFraC's electrostatic potential.}
%

\paragraph{Wt- and ReFraC titrate differently at low pH.}
%
...
\lumen{}, the close proximity of the like-charged and oppositely-charged residues respectively destabilzes and
...
\begin{table}
  ...
  \begin{tabularx}{9.5cm}{lSSSSSSS}
    ...
    & \multicolumn{2}{c}{$\Delta \pKa$$^\text{\emph{*}}$}
  ...
  \end{tabularx}
  \begin{flushleft}
    $^\text{\emph{*}}$$\pKa$-shift $\Delta \pKa = \pKa^{\textrm{effective}} - \pKa^{\textrm{baseline}}$.
  \end{flushleft}
\end{table}
%
...
\paragraph{Partial charges of Wt- and ReFraC at pH 4.5 and 7.5.}
%
...
\pHrange{7.5}{4.5}. This includes R10 in \gls{refrac}, which, in constrast to D10 in \gls{wtfrac}, keeps its
...
the constriction from negative to positve. Because arginine is significantly bulkier compared to glutamate,
...
of a nanopore constribute to the ion selectivity, not just those at its narrowest location. If we assume the
...
the net number of water molecules flowing through the pore ($\flux_{w}$) can be estimated using
\cite{Piguet-2014}
%
...
These velocities are comparable to literature values for pores of similar size
\cite{Boukhet-2016,Piguet-2014,Pederson-2015}.

Overall, these data confirm the experimental finding from Huang \etal{} (see~\cite{Huang-2017}), that, at
least for larger proteins such as chymotrypsin, the \gls{eof} drives their interactiong with \gls{frac}. The
...
\caption[Electrostatic energy of ssDNA translocation through Wt- and ReFraC.]{%
  ...
  location of the horizontal slices are indicated using a grey line (\sliceline).
  %
  Computations were performed using \gls{apbs} \cite{Baker-2001,Baker-2005}.
  %
  Molecular representations were rendered with \gls{vmd} \cite{Humphrey-1996,Stone-1998}.
...
\subsubsection{Translocation of ssDNA through Wt- and ReFraC.}
%

\paragraph{Calculating electrostatic energies using a ssDNA bead model.}
%
...
\paragraph{Reversing the electrostatic potential at the constriction significantly lowers the energy barrier.}
%
...
\paragraph{Overcoming the energy barriers inside the constriction.}
%
...
\paragraph{Precise manipulation of charges enables DNA translocation at physiological ionic strengths.}
%
As indicated in \cref{sec:np:clya}, the interior walls of \gls{clya} are lined predominantly with negatively
charged residues (cfr.~\cref{fig:clya_pore_section}), which prevents the pore from translocating \gls{dna} at
...
\caption[Electrostatic potential distribution inside several ClyA variants.]{%
  ...
  which were investigated for their ability to translocate DNA under physiological ionic strength in Ref.
  \cite{Franceschini-2016}. The labels indicate the mutations w.r.t. the \textit{S. typhi} wild-type (for AS),
  or w.r.t. \gls{clya-as} (for the others).
  %
...
\paragraph{All charges are important at physiological ionic strength.}
%
...
\paragraph{The constriction dominated at high ionic strengths.}
%
At \SI{2.5}{\Molar}, the overal electrostatic potential inside the pore drops to virtually \SI{0}{\kTe}
...
\paragraph{Computing the energy of DNA translocation with full-atom dsDNA model.}
%
...
sufficiently wide (\SI{33}{\angstrom}) to accomodate the full width of a \gls{dsdna} strand
(\SI{22}{\angstrom}).
...
\caption[Electrostatic energy analysis of dsDNA translocation through ClyA at physiological ionic strength.]{%
  ...
  (\SI{65}{\angstrom}, bottom). The location of the horizontal slices are indicated using a grey line
  (\sliceline).
  %
  Computations were performed using \gls{apbs} \cite{Baker-2001,Baker-2005}.
  %
...
\paragraph{DNA translocation at physiological ionic strengths is a fragile balance of energies.}
%
...
few \si{\kT} with a period of \SI{\approx30}{\angstrom}. This phenomenom is even more prominent at high salt
...
\caption[Electrostatic energy analysis of dsDNA translocation through ClyA at high ionic strength.]{%
  ...
  Once inside the \transi{} constriction, $\energyelec$ fluctutates periodically with a magnitude of
  ...
  (\SI{25}{\angstrom}, bottom). The location of the horizontal slices are indicated using a grey line
  (\sliceline).
  %
  Computations were performed using \gls{apbs} \cite{Baker-2001,Baker-2005}.
  %
...
\paragraph{High ionic strength smooths out and lowers energy barriers.} 
%
...
the constriction by almost an order of magnitude. This is in full agreement with the experimental obervations
of \gls{dsdna} translocation through \gls{clya-as} at higher ionic strengths
\cite{Franceschini-2013,Franceschini-2016}.
...
\caption[Electrostatic confinement of dsDNA within the constriction of ClyA-AS.]{%
  ...
  away from the axial center of ClyA-AS. The red crosses indicate the location of datapoints.
  %
...
coarse-grained \gls{md} simulations prsented in Bayoumi \etal{} \cite{Bayoumi-2020}, and was crucial to
properly model the behaviour of the \gls{ssdna}- and \gls{dsdna}-rotaxanes within \gls{clya}.

\paragraph{System setup and energy calculation.}
%
...
\paragraph{The energy mininum is not at the center.}
%
The contour plot of $\Delta \energyelec_{r=0}$ (\cref{fig:clya_dsdna_constriction_energy_heatmap}) reveals
that the energetic mininum for a piece of \gls{dsdna} does not lie at the center of the pore but is slightly
...
(\cref{clya_mutants_dsdna_0150_energy,fig:clya_mutants_dsdna_2500_energy}). 

\paragraph{Effective pore size.}
%
...
rise to increases of \SI{1}{\kT} and \SI{3}{\kT}, respectively. Asuming Boltzmann statistics, the probability
