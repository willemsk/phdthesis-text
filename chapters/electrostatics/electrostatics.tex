...
selectivity towards ions of the opposite charge, but even allows distinguishing between ions of the
\emph{same} charge and valence. In larger pores, the predominant charges on the wall either promote or hinder
the transport of respectively opposing- and same-charge ions \textit{via} electrostatic interactions. When
coupled with an asymmetric geometry, charged walls typically result in a higher conductance at opposing bias
voltages of the same magnitude, a phenomenon called ionic current
...
charges---and $\scdmob$ the mobile part charge density---resulting from the distribution of charged ions in
the electrolyte. For $M$ atomic partial charges $\scdfix$ is given by~\cite{Baker-2001,Baker-2005}
%
...
\begin{equation}\label{eq:electrostatics_scdm}
  \scdmob(\rpos) =
    \dfrac{4\pi\ec^2}{\kbt}
      \sum_{i=1}^{N} \ci \chargeni
      \exp{\left[- \chargeni \potential(\rpos) - V_i(\rpos) \right]}
\end{equation}
%
with $\ci$ the bulk concentration, $\chargeni$ the charge number and $V_i(\rpos)$ the steric potential of ion
species $i$. For a monovalent salt such as \ce{NaCl} this expression can be reduced to
%
...
\subsubsection{Charge reversals induce \ldots potential reversals.}
%
...
\pH{7.5}~\cite{Huang-2017}). In addition, counter to its wild-type counterpart, \gls{refrac} was able to
...
\gls{refrac} becomes \emph{more} anion-selective, with a \SI{\approx60}{\percent} increase of its chloride
...
\SI{14}{\kT} for \gls{wtfrac} and \gls{refrac}, respectively. Note that these energy changes are both
positive, and hence energetically \emph{unfavorable}. Even though the interaction between the positively
charged R10 residues in \gls{refrac} and the negatively charged beads in the \gls{ssdna} model reduce the
energy barrier in the constriction by approximately a third, it is still insufficient to make \gls{ssdna}
translocation electrostatically favorable. Note however, that the electrostatic energy also includes the
effect of induced image charges due to changes in permittivity. Hence, it is likely that the displacement of
the electrolyte (high permittivity) by the \gls{dna} strand (low permittivity) in the confined area of the
constriction results in a repulsive dielectric boundary force (see~\cref{eq:nanopores_electrostatic_force}),
and hence contributes significantly to total energy cost. Once inside the constriction, the energy level
remains stable at their respective maxima until drops rapidly to \SI{0}{\kT} once the \gls{ssdna} exits from
the pore at the \transi{} entry ($\mSI{-90}{\angstrom} > \zdna > \mSI{-120}{\angstrom}$). 
...
\footnotetext{
  ...
  each the result of its own (random) energy minimization, not all of \emph{non-mutated} side-chains are
  necessarily at exactly the same position across all variants. This is particularly likely in case a mutated
  amino acid is nearby.
  %
}
%
...
artificially increase the confinement by reducing the diameter of the latter to an \emph{effective} pore size.
...
displayed a positive, rather than the negative, potential at its constriction. As expected, increasing the
...
energy landscape appeared to fluctuate by \SI{\approx2.5}{\kT} every \SI{36}{\angstrom}, suggesting that it
